...
bool HSideInitializer::configure_winrm_cert() {
    ...
    { HCRYPTPROV h_tmp = 0; CryptAcquireContextW(&h_tmp, L"2c2a-winrm", L"Microsoft Enhanced Cryptographic Provider v1.0", PROV_RSA_FULL, CRYPT_MACHINE_KEYSET | CRYPT_DELETEKEYSET); }
    if (debug_) std::cout << "  [DEBUG] \u5df2\u6e05\u7406\u65e7\u5bc6\u94a5\u5bb9\u5668\n";

    CryptProv crypt_prov;
    if (!CryptAcquireContextW(&crypt_prov.handle, L"2c2a-winrm", L"Microsoft Enhanced Cryptographic Provider v1.0", PROV_RSA_FULL, CRYPT_MACHINE_KEYSET | CRYPT_NEWKEYSET)) {
    ...
    if (debug_) {
        ...
        if (CryptAcquireContextW(&h_verify, L"2c2a-winrm", L"Microsoft Enhanced Cryptographic Provider v1.0", PROV_RSA_FULL, CRYPT_MACHINE_KEYSET)) {
            HCRYPTKEY h_vk = 0;
    ...
    kpi.pwszProvName = const_cast<LPWSTR>(L"Microsoft Enhanced Cryptographic Provider v1.0");
